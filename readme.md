...
|----------|---------------------------------------------------------------------------|
| beta     | 启动器编写阶段，完全不可用                                                |
| preview  | 启动器测试阶段，已经部分可用但不建议用于日常（当前 v1.0.0-preview2 所处阶段） |
...
<details open>
<summary><h3>v1.0.0-preview2</h3></summary>

- **在线插件仓库整合进主窗口**：
  - 独立窗口改为插件管理页内的子视图：「在线插件仓库」按钮直接页内切换，Material 3 shared-axis 转场（300ms 淡入上浮），左上角「返回插件列表」切回
  - 安装期间切走页面**不再中断下载**（原独立窗口关闭即取消），切回时进度条仍在
  - 仓库索引懒加载：首次进入才请求远程索引，切回不重复请求
- **动画与微交互**：
  - 版本管理页切换实例时，详情面板新增 M3 上浮入场动画（仅用户主动点击时播放，默认选中保持静默加载）
  - 工作区设置的功能区卡片新增悬浮上浮微交互（与账号管理页同款）
- **关于页**：
  - 新增「加入我们」QQ 群卡片，一键复制群号
  - 「引用的库与资源」列表优化：补全 NAudio（音频后端）、修复空行、统一「版本 · 用途（许可证）+ 链接」格式
  - 移除版本号连点彩蛋（「秘密派对」整条链路删除；开发者名单彩蛋保留）
- **稳定性修复**：
  - 损坏的 Mod 元数据 JSON（如 `fabric.mod.json` 字符串内含真实换行符）不再导致实例详情页读取失败，降级为未知条目——对齐 PCL/HMCL 的宽容行为
- **日志**：
  - 日志同步镜像输出到控制台，便于调试与会话排查
  - 正式版（非 unstable 构建）的状态栏消息同样写入日志文件
- 移除了Herobrine
