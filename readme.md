...
- 插件列表可打开独立的 [NyaLauncher-Plugins](https://github.com/TouristH/NyaLauncher-Plugins) 在线仓库，按固定大小与 SHA-256 校验作者 GitHub Release 后安装；管理员审核且与版本哈希完全匹配的版本显示绿色标志，未经审核的版本下载前会显示风险确认。第三方作者可 Fork 该仓库并通过 Pull Request 申请收录。
...
- 修复个性化页面把仅注册于组件库、尚未放入工作区的插件来源误显示成第四个功能区的问题。内置、插件与用户新建功能区现在采用同一删除规则，可以全部移除直至工作区完全空白；空列表是明确的持久化状态，不会被后续注册自动补回，重置仍可恢复默认三功能区。
