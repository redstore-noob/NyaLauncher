...
它不仅轻量、快速，更注重 **隐私保护** 与 **界面自定义**，让你在享受游戏的同时，拥有完全自主的控制权。
...
- ⬡ **可扩展组件框架** —— 内置功能使用统一的声明式组件契约，可自由组合形状、内容与交互。
...
- `NyaLauncher.Plugin.Abstractions` 提供不依赖 Avalonia 的公共组件契约，让组件定义与启动器 UI 实现保持解耦。
...
- 工作区继续只按稳定组件 ID 保存所属功能区、相对坐标和层级，不持久化组件定义或瞬时进度。
